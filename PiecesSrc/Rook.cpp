...
std::vector<chess_move_t> Rook::allPossible(int x, int y, chess_board_t const& board) {
...
bool Rook::isPossible(int x, int y, int xTo, int yTo, chess_board_t const& board) {
    //If not in line
    if (x != xTo && y != yTo) {
        return false;
    }
